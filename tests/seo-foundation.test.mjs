...
test("production observability is wired into the root layout", async () => {
  ...
  assert.match(llmsFull, /https:\/\/washop\.co\.il\/category\/local-services/);
...
test("the virtual-store guide owns the observed online-store intent", async () => {
  const blog = await readFile("src/data/blog.ts", "utf8");

  assert.match(blog, /title: "איך פותחים חנות מקוונת או וירטואלית בוואטסאפ\?"/);
  assert.match(blog, /metaTitle: "חנות מקוונת או וירטואלית בוואטסאפ: איך מתחילים\? \| WaShop"/);
});
